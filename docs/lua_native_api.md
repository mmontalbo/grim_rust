...
- Builds the `system` table, logs creation, stores it as ref 0, then retrieves and attaches `controls`, native handler closures for `camChangeHandler`/`axisHandler`/`inputModeHandler`/`buttonHandler`, `setTable`, and `currentActor` (with `set_selected` / `default` / `put_in_set` stubs) to mirror retail sequencing.
