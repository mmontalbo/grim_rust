...
- A `system` table is created and bound as a global; retail stores it as ref 0 and later retrieves it to attach `controls` and other fields via `settable`. Subtables like `system.controls` and actor stubs (`currentActor` / `manny` functions) are populated during bootstrap.
...
- Registers math/string aliases, platform/config helpers, input control toggles, registry access shims, and `break_here`; hardcodes sector mode constants and `PI`.
- Builds the `system` table, logs creation, stores it as ref 0, then retrieves and attaches `controls`, `setTable`, and `currentActor` (with `set_selected` / `default` / `put_in_set` stubs) to mirror retail sequencing.
